import { ArrowRight, Heart, ShoppingCart, Sparkles, Star } from "lucide-react";
import { Link } from "react-router-dom";

export function NewArrivalsSection({ products = [] }) {
  return (
    <section className="p-20">
      <div className="container-custom">
        <div className="mb-12 flex flex-col items-start justify-between md:flex-row md:items-center">
          <div>
            <span className="mb-2 inline-block rounded-full px-3 py-1 text-sm font-semibold">
              <Sparkles className="mr-1 inline h-4 w-4" /> This Week&apos;s
              Arrivals
            </span>
            <h2 className="font-heading text-4xl font-bold text-gray-800 dark:text-zinc-200">
              Fresh This Week
            </h2>
            <p className="mt-2 text-lg text-gray-600 dark:text-zinc-200">
              The latest products added to our store this week
            </p>
          </div>

          <Link
            to="/products?sort=newest"
            className="text-primary-600 hover: animated-underline mt-4 inline-flex items-center md:mt-0"
          >
            View All New Arrivals <ArrowRight className="ml-2 h-4 w-4" />
          </Link>
        </div>

        <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 md:grid-cols-4">
          {products.map((product, index) => (
            <div
              key={product.id}
              className="product-card-enhanced group animate-fade-in"
              style={{ animationDelay: `${index * 150}ms` }}
            >
              <div className="relative overflow-hidden rounded-t-xl">
                <span className="absolute top-3 right-0 z-10 rounded-l-lg bg-green-500 px-2 py-1 text-sm font-bold text-white uppercase shadow-md">
                  Just Added
                </span>

                <Link to={`/products/${product.id}`} className="block">
                  <img
                    src={product.image || "/placeholder.svg"}
                    alt={product.name}
                    className="h-64 w-full object-cover object-center transition-transform duration-500 group-hover:scale-110"
                  />
                </Link>

                <div className="absolute right-0 -bottom-10 left-0 flex justify-center space-x-2 bg-white/80 p-2 backdrop-blur-sm transition-all duration-300 group-hover:bottom-0">
                  <button className="bg-primary-500 hover:bg-primary-600 rounded-full p-2 text-white shadow transition-transform hover:scale-110">
                    <ShoppingCart className="h-4 w-4" />
                  </button>
                  <button className="text-primary-500 hover:text-primary-600 rounded-full bg-white p-2 shadow transition-transform hover:scale-110">
                    <Heart className="h-4 w-4" />
                  </button>
                </div>
              </div>

              <div className="bg-white p-4">
                <h3 className="mb-1 text-lg font-medium text-gray-900">
                  {product.name}
                </h3>
                <div className="mb-2 flex items-center">
                  {Array.from({ length: 5 }).map((_, index) => (
                    <Star
                      key={index}
                      className={`h-4 w-4 ${
                        index < Math.floor(product.rating)
                          ? "fill-yellow-400 text-yellow-400"
                          : "fill-gray-200 text-gray-200"
                      }`}
                    />
                  ))}
                  <span className="ml-1 text-xs text-gray-500">
                    ({Math.floor(product.rating)})
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-lg font-bold text-gray-900">
                    ${product.price}
                  </span>
                  {product.brand && (
                    <span className="text-xs text-gray-500">
                      {product.brand.name}
                    </span>
                  )}
                </div>
              </div>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
}
