import { Button, Card, Grid, Heading, Spinner, Text } from "@components";
import {
  CreditCard,
  Package,
  ShoppingBag,
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            {trend && (
              ...
              >
                <TrendingUp
                  className={`mr-1 h-4 w-4 ${trend < 0 ? "rotate-180" : ""}`}
                />
                {trend > 0 ? "+" : ""}
          ...
          <div
            className={`rounded-full p-4 ${colorClasses[color] || colorClasses.purple}`}
          >
            {icon}
          </div>
        </div>
...
const DashboardHomePage = () => {
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        if (user?.role === USER_ROLES.ADMIN) {
          ...
          try {
            storeProductsRes = await productService.getProducts({
              store_owner: user.id,
              limit: 1,
            });
          } catch (error) {
          ...
          setStats({
            ...
            totalSpent: (orderHistoryRes.data.results || []).reduce(
  ...
  return (
    <div>
      ...
      {/* Stats Cards */}
      <Grid
        cols={user?.role === USER_ROLES.CUSTOMER ? 2 : 4}
        gap={6}
        className="mb-8"
      >
